...
* 会話は、相手が同作か否かにかかわらず、一般的なネットワーク越し（双方のキャラ自身がTwitterあるいは類似の手段を利用している）前提でのやり取りではなく、すべて仮想のシーンとシチュエーションによる擬似直接対面形式で行う。ストーリー偏重で、その真逆といえる取り留めのない無目的のやり取りはキャラらしくなく演技方針上も好ましくないため、可能なれど消極的。
